...
script.on_event(defines.events.on_research_started, function(event)
  game.write_file(filename, "s " .. event.research.name .. "\n", true, 0)
  global.lastProgressCheckAmount = game.forces['player'].research_progress
end)

script.on_event(defines.events.on_research_finished, function(event)
  game.write_file(filename, "f " .. event.research.name .. "\n", true, 0)
  global.lastProgressCheckAmount = 0
end)

remote.remove_interface("researchSync")
remote.add_interface("researchSync", {
  updateProgress = function(researchName, newProgress)
    if game.forces['player'].current_research and game.forces['player'].current_research.name == researchName then
